""" Audio Processing Module Handles audio search and download functionality. """

import os import logging import asyncio import yt_dlp from typing import Dict, Optional import tempfile import hashlib

logger = logging.getLogger(name)

class AudioProcessor: """Handles audio search and download operations."""
...
async def download_track(self, track_info: Dict, quality: str) -> Optional[str]:
    ...
    # Try SoundCloud first, then YouTube Music
    search_prefixes = ["scsearch1", "ytmusicsearch1"]

    for prefix in search_prefixes:
        try:
            logger.info(f"Trying: {prefix}:{search_query}")
