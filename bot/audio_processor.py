...
class AudioProcessor:
    ...
    async def download_track(self, track_info: Dict, quality: str) -> Optional[str]:
        """
        Download track audio file with fallback search sources.
        """
        try:
            search_queries = [
                f"scsearch1:{track_info['name']} {track_info['artist']}",  # SoundCloud
                f"ytsearch1:{track_info['name']} {track_info['artist']}"   # YouTube
            ]
            ...
            logger.error(f"Download failed for all sources: {track_info['name']} {track_info['artist']}")
            return None
    ...
    def _download_audio(self, search_query: str, ydl_opts: Dict) -> Optional[str]:
        try:
            with yt_dlp.YoutubeDL(ydl_opts) as ydl:
                search_results = ydl.extract_info(f"{search_query}", download=False)
